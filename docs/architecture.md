...
Only the homepage route exists. The project links still open Behance. Dedicated case studies, additional pages, advanced 3D, CMS editing, final SEO/social cards, and publishing belong to later milestones. The intro is a decorative timed sequence and holds the scroller until its curtain clears. The HM scene remains a transformed image, now swept in three dimensions across a taller stage rather than rendered in WebGL. Header contrast retains the prototype's gradient thresholds; it now expresses its result with a themed data attribute.
