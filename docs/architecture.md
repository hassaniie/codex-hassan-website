...
Only the homepage route exists. The project links still open Behance. Dedicated case studies, additional pages, advanced 3D, CMS editing, final SEO/social cards, and publishing belong to later milestones. The intro is a decorative timed sequence and holds the scroller until its curtain clears. It is shell behavior, so every route opens the same way, and every arrival plays it whole: the loader counts to a hundred, sits there, dissolves, and the curtain lifts off the bottom edge partway through that dissolve, on a curve fitted to the approved source rather than chosen. An inline script arms it before first paint so content never flashes ahead of it; that script opts out entirely under reduced motion and drops its class on a timer, so a bundle that never runs cannot leave the page hidden. A page change is the same sequence rather than a shorter one. Leaving a route puts the curtain up in a single frame with no wipe, and resets the count to zero as it goes, so the page being left never shows the hundred it finished on; the next document then loads behind an identical curtain and runs the one count. A cut is what makes that seam invisible: there is no wipe on the departing page whose position the arriving one would have to match, which is how the change stays continuous without introducing a client router. The process rail below Experience is the only horizontal motion on the page and the only pinned section in the codebase: ScrollTrigger holds it for exactly the width of its own track and spends that scroll translating the track sideways, so the section is read across rather than down. Lenis drives the real scroll position rather than a transformed wrapper, and nothing in the document creates a containing block, so the pin works through the default fixed strategy and no scroller proxy is registered; adding one would be a regression, not a fix. Which mode the rail runs in is decided once in CSS and read back through a custom property, and the pinned presentation is applied by a class the scene itself adds. The baseline underneath is therefore a plain scrollable rail with snap points, which is what touch, reduced motion and a document whose script never runs all get, with every panel reachable in each. Nothing in the rail is focusable, so keyboard travel cannot pull the page to a panel that is still off screen. The cursor dot is an accent over the native pointer rather than a replacement for it, damped to trail the pointer and drawn with difference blending so it stays visible on any background; it is hidden on touch layouts and under reduced motion. The HM scene remains a transformed image, now swept in three dimensions across a taller stage rather than rendered in WebGL. Header contrast retains the prototype's gradient thresholds; it now expresses its result with a themed data attribute.
